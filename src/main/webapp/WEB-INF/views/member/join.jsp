		...
		$("form")
				.submit(
						function() {
							...
							//필수 항목 체크
							$(":text,:password").filter(".validate").each(
							function() {
							$(this).filter(".required").each(
							function() {
							if ($(this).val() == "") {
							$(this).parent().append(
							"<p class='error'>필수항목입니다</p>");
							}
							});
							});
							...
							/* if ($("#id").val() == ($("#pw1").val())) {
								alert("ID와 동일한 비밀번호는 사용하실 수 없습니다.");
								$("#pw1").val("");
								$("#pw1").focus();
							} */
...
<body>
	<div class="d-flex justify-content-around h-100">
		<a href="/Project_UDB0526" style="color: green; font-size: 30px;"><img
	...
	<div class="d-flex justify-content-center" id="container">
		<div id="content">
			<form action="join" method="post">
				...
				<div>
					<label>이름<span> ※ </span></label><br /> <input type="text"
				...
				<div>
					<label>이메일 입력<span> ※ </span></label><br /> <input type="text"
						name="email" id="email" size="35" class="validate required" />
				</div>
				<hr />

				<label>주소 입력<span> ※ </span></label><br /> <input type="text"
				...
				<input type="text" id="msaddr" name="msaddr" placeholder="주소"
					...
					id="msextraaddress" name="msextraaddress" placeholder="참고항목"
				...
				<hr />
				<div>
					<label>전화번호 입력<span> ※ </span></label><br /> <input type="text"
						name="mstel" id="mstel" size="35" class="validate required" />
				</div>
				<hr />

				<div>
					<label>생일 입력<span> ※ </span></label><br /> <input type="text"
						name="msbirth" id="msbirth" class="validate required"
						value="2020-01-01" readonly />
				</div>
				<hr />

				<div>
					<label>성별 입력<span> ※ </span></label>
					<div>
						<select name="msgender" id="msgender" class="validate required">
				...
				<p style="text-align: center">
					...
					<input class="btn btn-outline-success" type="button" value="취소"
						onclick="location.href='/Project_UDB0520/'" />
				</p>
			</form>
		</div>
	</div>
			...
			new daum.Postcode(
					{
						oncomplete : function(data) {
							...
							if (data.userSelectedType === 'R') {
								...
								document.getElementById("msextraaddress").value = extraAddr;

							} else {
								document.getElementById("msextraaddress").value = '';
							}
							...
							// 커서를 상세주소 필드로 이동한다.
							document.getElementById("msdetailaddress").focus();
						}
					}).open();
		}
	</script>
