...
//		int msno = Integer.parseInt(request.getParameter("0"));
		...
		String msbirth = request.getParameter("msbirth");
		String msgender = request.getParameter("msgender");
...
//		int result = dao.join(new MemberDTO(0, msid, mspw, msname, msemail, SHA256.getSHA256(msemail), 0,msaddr , mstel, msbirth, msgender, "seeker",mspostcode,msdetailaddress,msextraaddress));

		int result = dao.join(new MemberDTO(0, msid, SHA256.getSHA256(mspw), msname, msemail, SHA256.getSHA256(msemail), 0, msaddr, mstel, msbirth, msgender, "seeker", mspostcode, msdetailaddress, msextraaddress, ""));
	...
	private void emailSendAction(String id) {
		String host = "http://localhost:9000/project_UDB2/";
